namespace WixToolset.Samples.EmbeddedUI
{
    ...
    public class SampleEmbeddedUI : IEmbeddedUI
    {
        ...
        private string ReadmeText { get; set; }
        ...
        public bool Initialize(Session session, string resourcePath, ref InstallUIOptions internalUILevel)
        {
            if (session != null)
            {
                string sql = "SELECT `Data` FROM `Binary` WHERE `Name` = 'ReadmeFile'";
                using (View view = session.Database.OpenView(sql))
                {
                    ...
                    if (record != null)
                    {
                        Stream data = record.GetStream(1);
        ...
        private void Run()
        {
            ...
            setupWizard.messagesTextBox.Text = ReadmeText;
